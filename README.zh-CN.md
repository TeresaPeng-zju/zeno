...
5. **把成长铸在链上（AI×Web3）** —— 一键把这段迁移旅程铸成 Soulbound 迁移护照（ERC-5192 不可转让），部署于 Base Sepolia，图与元数据完全链上渲染。每枚护照都有公开验证页（`/passport/[id]`），数据直接读自区块链——任何人无需信任 Zeno 的服务器即可验证。随着成长重新铸造，同一枚护照会持续进化。
